...
INPUT_DIR = "DataSuds.IRD_JSONs" 
# Nom du fichier de mapping généré
MAPPING_FILE = "MappingIRD.json"
# Nom du fichier CSV de sortie
OUTPUT_CSV_FILE = "output_mapping_IRD.csv"
...
def process_mapping(data: Dict[str, Any], mapping_config: List[Dict[str, str]]) -> Dict[str, Any]:
    ...
    # Ajoute le préfixe 'IRD' à la colonne 'keyword'.
    ...
    # --- LOGIQUE D'AJOUT DU PRÉFIXE 'IRD' APRÈS TOUTES LES FUSIONS ---
    
    for column_name, value in mapped_data.items():
        if column_name == "keyword":
            # Si la valeur est vide, on ajoute juste "IRD"
            if not value.strip():
                mapped_data[column_name] = "IRD"
            # Si la valeur contient déjà des mots-clés, on ajoute "IRD" suivi du séparateur
            else:
                prefix = "IRD" + KEYWORD_SEPARATOR
                # Vérifier si "IRD" est déjà présent au début pour éviter la duplication
                if not value.startswith("IRD"):
